...
import KelasScheduleTable from './KelasScheduleTable';
...
const ProductDetail = () => {
        ...
        try {
            ...
            if (type === 'inventaris') {
                ...
                itemData = {
                    ...
                    currentStock: response.data.status === 'tersedia' ? 0 : 1,
                    maxStock: 1,
    ...
    // For regular inventaris only (ruangan akan pakai scheduling)
    const isAvailable = isInventaris && item && item.currentStock < item.maxStock;
    ...
    const handleSubmitRequest = () => {
        ...
        console.log('Submitting request for:', item);
        alert(`Mengajukan peminjaman untuk ${item.name}`);
    };

    const handleRequestAbsen = (course) => {
        console.log('Requesting absen for course:', course);
        alert(`Mengajukan peminjaman absen untuk mata kuliah: ${course.name}`);
    };

    // NEW: Handler for kelas booking
    const handleRequestKelas = (bookingData) => {
        console.log('Requesting kelas booking:', bookingData);
        alert(`Mengajukan peminjaman kelas dari ${bookingData.waktu_mulai} - ${bookingData.waktu_selesai} pada ${bookingData.tanggal}`);
    };
                    ...
                    <div className="bg-white rounded-2xl border border-red-700 p-8 lg:p-12">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Status badge - hanya untuk inventaris */}
                                {isInventaris && (
                                ...
                                {/* Stock info - hanya untuk inventaris */}
                                {isInventaris && (
